...
function ReviewSession() {
  ...
  const quickNotesOptions = [
    "ممتاز - يواصل بنفس الوتيرة",
    "جيد - يحتاج تحسين التجويد",
    "يحتاج مراجعة إضافية",
    "انتبه للأخطاء المتكررة",
    "تحسن ملحوظ عن المرة السابقة",
  ...
  useEffect(() => {
    ...
    const fetchCompletions = async () => {
      try {
        ...
        setCompletions(reviewSessions);

        // Extract completed Surahs or Hizbs from REVIEW sessions only
        if (isChapterMethod) {
          const completed = reviewSessions
            .filter((c) => c.is_surah_completed && c.surah)
            .map((c) => c.surah.code);
          setCompletedSurahs(completed);
        } else {
          // Extract completed Hizbs from REVIEW sessions only
          const completed = reviewSessions
  ...
  const handleSave = async () => {
    ...
    if (isChapterMethod) {
      ...
      if (!reviewData.verseFrom || !reviewData.verseTo) {
        setDialog({
          open: true,
          type: "error",
          title: "خطأ في البيانات",
      ...
      if (!reviewData.hizbNumber || !reviewData.eighthNumber) {
        setDialog({
          open: true,
          type: "error",
          title: "خطأ في البيانات",
    ...
    try {
      const payload = isChapterMethod
        ? {
            session_type: "review",
            chapter: parseInt(reviewData.surah), // chapter is required by backend
            surah: parseInt(reviewData.surah),
            verse_from: parseInt(reviewData.verseFrom),
            verse_to: parseInt(reviewData.verseTo),
            is_surah_completed: reviewData.isSurahCompleted,
            next_surah: reviewData.nextSurah
              ? parseInt(reviewData.nextSurah)
              : null,
            next_verse_from: reviewData.nextVerseFrom
              ? parseInt(reviewData.nextVerseFrom)
              : null,
            next_verse_to: reviewData.nextVerseTo
              ? parseInt(reviewData.nextVerseTo)
              : null,
            completion_date: reviewData.completionDate,
            rating: reviewData.rating,
            quick_notes: reviewData.quickNotes,
            evaluation: reviewData.evaluation,
            feedback: reviewData.feedback,
          }
        : {
            session_type: "review",
            hizb_number: parseInt(reviewData.hizbNumber),
            eighth_number: parseInt(reviewData.eighthNumber),
            is_hizb_completed: reviewData.isHizbCompleted,
            next_hizb_number: reviewData.nextHizbNumber
              ? parseInt(reviewData.nextHizbNumber)
              : null,
            next_eighth_number: reviewData.nextEighthNumber
              ? parseInt(reviewData.nextEighthNumber)
              : null,
            completion_date: reviewData.completionDate,
            rating: reviewData.rating,
            quick_notes: reviewData.quickNotes,
      ...
      setDialog({
        ...
        message:
          reviewData.isSurahCompleted || reviewData.isHizbCompleted
            ? "🎉 تهانينا! تم إكمال السورة/الحزب وحفظها في إنجازات الطالب"
            : "تم حفظ حصة المراجعة بنجاح!",
      });
        ...
        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={
                        reviewData.surah
                          ? getSelectedSurahInfo(reviewData.surah)
                              ?.number_of_verses
                          : ""
                      }
                      className="w-full text-center"
                  ...
                  <div className="flex items-center gap-2 md:col-span-1">
                    <input
                      type="checkbox"
                      id="surahCompleted"
                      checked={reviewData.isSurahCompleted}
                      onChange={(e) =>
                        setReviewData({
                          ...reviewData,
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      min="1"
                      value={reviewData.nextVerseFrom}
                      onChange={(e) =>
                        setReviewData({
                          ...reviewData,
                  ...
                  <div className="md:col-span-2">
                    ...
                    <Input
                      ...
                      placeholder={
                        reviewData.nextSurah
                          ? getSelectedSurahInfo(reviewData.nextSurah)
                              ?.number_of_verses
                          : ""
                      }
                      className="w-full text-center"
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox"
                      id="hizbCompleted"
                      checked={reviewData.isHizbCompleted}
                      onChange={(e) =>
                        setReviewData({
                          ...reviewData,
              ...
              <div className="mb-6 border-b pb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select
                      value={reviewData.nextHizbNumber}
                      onChange={(e) =>
                        setReviewData({
                          ...reviewData,
                  ...
                  <div>
                    ...
                    <Select
                      value={reviewData.nextEighthNumber}
                      onChange={(e) =>
                        setReviewData({
                          ...reviewData,
          ...
          <div className="mb-4">
            ...
            <Input
              ...
              onChange={(e) =>
                setReviewData({ ...reviewData, completionDate: e.target.value })
              }
          ...
          <div className="mb-4">
            ...
            <p className="text-center font-cairo text-xs sm:text-sm text-gray-600 mt-2">
              {getRatingLabel(reviewData.rating)}
            </p>
          </div>
          ...
          <div className="mb-4">
            ...
            <Select
              value={reviewData.quickNotes}
              onChange={(e) =>
                setReviewData({ ...reviewData, quickNotes: e.target.value })
              }
              className="w-full text-right"
              dir="rtl"
            >
          ...
          {completions.length > 0 && (
            <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-3">
              <p className="text-center font-cairo text-sm text-green-800">
                {isChapterMethod
                  ? `تم تسجيل ${completions.length} حصة مراجعة${
